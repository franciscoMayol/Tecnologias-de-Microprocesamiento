...
int main(void)
{
	...
	longitud_clave = EEPROM_read(0x00);
	/*//Para sumulacion
	clave[0] = 1;
	clave[1] = 2;
	clave[2] = 3;
	clave[3] = 4;
	//
	*///Para inicar la contraseña en la EEPROM la primer vez
	//EEPROM_write(0x10, 1);
	//EEPROM_write(0x11, 2);
	//EEPROM_write(0x12, 3);
	//EEPROM_write(0x14, 4);
	...
	while (1) {
		...
		longitud_clave = EEPROM_read(0x00); //Para simulacion, a 4, 5 o 6 dígitos
		int posicion = 0;
		char caracter;
